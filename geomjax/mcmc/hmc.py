...
class hmc:
    ...
    The general hmc kernel builder (:meth:`geomjax.mcmc.hmc.build_kernel`, alias `geomjax.hmc.build_kernel`) can be
    cumbersome to manipulate. Since most users only need to specify the kernel
    parameters at initialization time, we provide a helper function that
    specializes the general kernel.

    We also add the general kernel and state generator as an attribute to this class so
    users only need to pass `geomjax.hmc` to SMC, adaptation, etc. algorithms.
    ...
    .. code::
        ...
        hmc = geomjax.hmc(logdensity_fn, step_size, inverse_mass_matrix, num_integration_steps)
    ...
    .. code::
       ...
       kernel = geomjax.hmc.build_kernel(integrators.mclachlan)
       state = geomjax.hmc.init(position, logdensity_fn)
